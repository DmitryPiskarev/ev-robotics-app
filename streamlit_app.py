...
def suspension_plotly(phi_deg, l_lca, l_uca, inner_dist, ang_deg, outer_dist,
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=[LCA_in[0], LCA_out[0]], y=[LCA_in[1], LCA_out[1]], mode="lines+markers", name="LCA"))
    fig.add_trace(go.Scatter(x=[UCA_in[0], UCA_out[0]], y=[UCA_in[1], UCA_out[1]], mode="lines+markers", name="UCA"))
    fig.add_trace(go.Scatter(x=[LCA_out[0], UCA_out[0]], y=[LCA_out[1], UCA_out[1]], mode="lines+markers", name="Knuckle"))
    fig.add_trace(go.Scatter(x=[inner[0], outer[0]], y=[inner[1], outer[1]], mode="lines+markers", name="Tie-Rod"))
...
def optimize_suspension(l_lca, l_uca, outer_dist, inner_dist,
    ...
    def objective(vars):
        ...
        ix = inner_x if inner_fixed is None else inner_fixed
        iy = inner_y if inner_y_fixed is None else inner_y_fixed
        t  = t_pickup if tie_fixed is None else tie_fixed
        ang = ang_deg if ang_fixed is None else ang_fixed
        off = offset_dist if offset_fixed is None else offset_fixed
        _, dev = tie_rod_deviation((ix, iy), t, l_lca, l_uca, inner_dist, ang, outer_dist, off)
        return np.max(np.abs(dev))

    x0 = [0.0 if inner_fixed is None else inner_fixed,
    ...
    bounds = [(-50,50) if inner_fixed is None else (inner_fixed, inner_fixed),
...
with st.sidebar:
    ...
    inner_x = st.slider("Inner pivot X [mm]", -50.0, 50.0, 4.6, 0.1, key="inner_x")
    inner_y = st.slider("Inner pivot Y [mm]", 0.0, 80.0, 19.3, 0.1, key="inner_y")
    t_pickup = st.slider("Pickup (0=LCA, 1=UCA)", 0.0, 1.0, 0.36, 0.01, key="t_pickup")
    offset_dist = st.slider("Offset distance [mm]", -20.0, 20.0, -2.6, 0.1, key="offset_dist")
    ang_deg = st.slider("Chassis pivot inclination [deg]", -20.0, 20.0, -9.8, 0.1, key="ang_deg")
